...
function createVariable(sv) {
	...
	sv.arr.TenChucNang = ["Họ và tên", "Email", "Mobile"];
	sv.arr.HintText = ["Nhập họ và tên của bạn", "kimnamcham@gmail.com", "Nhập số di động của bạn"];
	sv.vari.lineRow1 = [];
}
...
function createUI_Event(sv) {
	...
	sv.fu.evt_btnXacNhan = function(e) {
		...
		sv.vari.db.close();
		if (sv.vari.txtChucNang1[0].value == "" || sv.vari.txtChucNang1[1].value == "" || sv.vari.txtChucNang1[2].value == "") {
			alert("Bạn chưa nhập đầy đủ thông tin vào");
		} else {
			thaydoi_info({
				"username" : sv.vari.ten_user,
				"email" : sv.vari.txtChucNang1[1].value,
				"mobile" : sv.vari.txtChucNang1[2].value,
				"fullname" : sv.vari.txtChucNang1[0].value
			}, sv);
		}
...
function thaydoi_info(data, sv) {
	...
	xhr.onload = function() {
		...
		if (jsonResuilt.code == 0) {
			...
			alert("Thay đổi thông tin cá nhân thất bại");
		}
	};
